...
#>
    ...
    process {
        ...
        foreach ($xmldoc in $InputObject) {
            ...
            $itotal = $nmaprun.runstats.hosts.total
...
<#TODO: Reimplement PATH functinoality
...
#>
		    ...
		    $stat.VerboseLevel = $nmaprun.verbose.level
		    $stat.DebuggingLevel = $nmaprun.debugging.level		
		    $stat.HostsUp = $nmaprun.runstats.hosts.up
		    $stat.HostsDown = $nmaprun.runstats.hosts.down		
		    $stat.HostsTotal = $nmaprun.runstats.hosts.total
		    $stat 			
	    }
    ...
    ForEach ($file in $Path) {
	    ...
	    # Process each of the <host> nodes from the nmap report.
	    $i = 1  #Counter for <host> nodes processed.
        $itotal = @($nmaprun.host).count
	    $nmaprun.host | foreach-object { 
